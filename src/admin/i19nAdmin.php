...
class i19nAdmin extends LeftAndMain implements PermissionProvider
{
        ...
        $editable->setDisplayFields([
            'Value' => [
                ...
                'callback' => function ($record, $column, $grid) {
                    return TextField::create($column)->setTitle('Vrednost');
                },
            ]
        ]);
        ...
        $form = Form::create(
            ...
            FieldList::create([
                FormAction::create('save')
                    ->setTitle(_t(__CLASS__ . '.BUTTON_SAVE_RECORDS', __CLASS__ . '.BUTTON_SAVE_RECORDS'))
    ...
    public function save($data, $form)
    {
        ...
        $response->addHeader('X-Status', _t(__CLASS__ . '.ACTION_SAVED', __CLASS__ . '.ACTION_SAVED'));
        return $response;
    }
    ...
    public function ImportForm(): Form
    {
        $fields = FieldList::create([
            FileField::create('ymlFile')
                ->setTitle(_t(__CLASS__ . '.IMPORT_UPLOAD_YML', __CLASS__ . '.IMPORT_UPLOAD_YML'))
                ->setAllowedExtensions(['yml']),
            CheckboxField::create('UpdateExisting')
                ->setTitle(_t(__CLASS__ . '.IMPORT_UPDATE_EXISTING', __CLASS__ . '.IMPORT_UPDATE_EXISTING'))
                ->setValue(false)
        ]);
        
        $actions = FieldList::create(
            FormAction::create('importYML', _t(__CLASS__ . '.BUTTON_DO_IMPORT', __CLASS__ . '.BUTTON_DO_IMPORT'))
    ...
    public function importYML($data, $form, $request)
    {
        if (empty($_FILES['ymlFile']['tmp_name']) ||
            ...
            $form->sessionMessage(
                _t(__CLASS__ . '.IMPORT_NO_YML_FILE', __CLASS__ . '.IMPORT_NO_YML_FILE'),
        ...
        if (!is_array($yaml) || count($yaml) < 1) {
            $form->sessionMessage(
                _t(__CLASS__ . '.IMPORT_NO_DATA', __CLASS__ . '.IMPORT_NO_DATA'),
                ValidationResult::TYPE_ERROR
            );
            $this->redirectBack();
            return false;
        ...
        foreach ($yaml as $locale => $list_translations) {
            ...
            if (!$db_locale) {
                $form->sessionMessage(_t(__CLASS__ . '.SAVED_ACTION', __CLASS__ . '.SAVED_ACTION', ['inserted' => $count_inserts, 'updated' => $count_updates, 'file_name' => $_FILES['ymlFile']['name']]), 'good');
